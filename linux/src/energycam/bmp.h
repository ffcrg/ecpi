...
// Offset of the file size field in BMP header
#define BMP_HEADER_FIELD_FILE_SIZE  2
// Offset of data offset field in BMP header
#define BMP_HEADER_FIELD_DATA_OFFSET  10
// Offset of height field in BMP header
#define BMP_HEADER_FIELD_HEIGHT 22
// Offset of width field in BMP header
#define BMP_HEADER_FIELD_WIDTH 18
// Offset of width field in BMP header
#define BMP_HEADER_FIELD_COLOR_DEPTH 28
// Offset of image size field in BMP header
#define BMP_HEADER_FIELD_IMAGE_SIZE 34

// color depth of a RGB image in the BMP header
#define BMP_BPP_RGB             24

#define ST_INT32(pData, val) \
{ \
	((pData)[0] = (uint8_t)( val & 0x000000FF));         \
	((pData)[1] = (uint8_t)((val & 0x0000FF00) >> 8));   \
	((pData)[2] = (uint8_t)((val & 0x00FF0000) >> 16));  \
	((pData)[3] = (uint8_t)((val & 0xFF000000) >> 24));  \
}

#define ST_INT16(pData, val) \
{ \
	((pData)[0] = (uint8_t)( val & 0x00FF));            \
	((pData)[1] = (uint8_t)((val & 0xFF00) >> 8));      \
}




// definition of a generic bmp header for greyscale images.
uint8_t aryBmpHeadGrey[] = {
	/* 0     1     2     3     4     5     6     7    */
...
void BmpWriteHdrInfo(uint8_t *pHdr,const int width, const int height, const short colorDepth,	const int headerSize)
{
	int imageSize, dataOffset, fileSize;

	imageSize = ((int)width*(colorDepth/8) + ((int)width*(colorDepth/8))%4) * height;
